...
_interrupt_handler:
	...
	PUSHAQ		; Pushes all callee-preserved registers

	cld             ; C code following the sysV ABI requires DF to be clear

	mov rdi, rsp	; rdi = stack = arguments

	call interrupt_handler ; Call the real interrupt handler

	POPAQ		; restore registers

	add rsp, 0x10	; pop the error_code and interrupt number from stack
			; (IRETQ does not pop the error code)

	sti		; re-enable interrupts

	iretq		; return from AMD64 interrupts
...
interrupt_handler_%1:
	push qword 0x00		; Empty error code
...
global interrupt_handler_ec_%1
interrupt_handler_ec_%1:
	push qword %1		; Interrupt Number
	jmp _interrupt_handler
%endmacro
...
; General use interrupt_handlers
